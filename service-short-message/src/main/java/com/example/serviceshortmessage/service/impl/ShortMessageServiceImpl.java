package com.example.serviceshortmessage.service.impl;
...
import com.example.serviceshortmessage.constant.VerificationCodeType;
import com.example.serviceshortmessage.properties.ShortMessageProperties;
...
//        //初始化acsClient,暂不支持region化
    ...
    public void sendShortMessageFromCode(String phoneNumbers, String code, String codeType) {
        ...
        try {
            ...
            if (Objects.equal(codeType, VerificationCodeType.REGISTER.getType())) {
                templateCode = properties.getRegisterTemplateCode();
            }
            if (Objects.equal(codeType, VerificationCodeType.LOGIN.getType())) {
                templateCode = properties.getLoginTemplateCode();
            }
            if (Objects.equal(codeType,VerificationCodeType.FORGET.getType())) {
        ...
        if (response == null || !Objects.equal(JSONObject.parseObject(response.getData()).getString("Code"), "OK")) {
            throw new RuntimeException("短信服务异常，请联系管理员");
        }
